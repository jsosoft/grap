...
(function()
{
	JSO.Client.Controller.Emitter = JSO.Client.Controller.extend({
		...
		initialize: function(){
			...
			if (this.model.cached == true) this.fillParticleCache();
		...
		emit: function(){
			...
			if (this.model.cached === true && this.particleLoopBuffer.length > 0) {
				particleSet = this.particleLoopBuffer.pop();
			} else {
				particleSet = this.getParticleSet();
			}
			
			JSO.Client.Globals.gameController.executionQueues.push(new JSO.Client.ExecutionQueue(this.emitParticle, this, particleSet, null, null, 30));
