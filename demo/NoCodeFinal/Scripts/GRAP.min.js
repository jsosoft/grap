...
return parent;}﻿
var JSO=namespace('JSO');var JSO_Client=namespace('JSO.Client');var DEBUG=false;if(typeof exports!='undefined'){JSO.Client=exports;}else{JSO.Client=this.JSO.Client={};}
...
this.mainCam=JSO.Client.Globals.gameController.cameras[0].model;this._super();},update:function(){var vel=this.velocity.Copy();var rot=this.rotation*(Math.PI/180);if(this.acceleration.x>0||this.acceleration.y>0||this.acceleration.x<0||this.acceleration.y<0)vel=this.velocity.Add(this.acceleration);if(vel.x<.01&&vel.x>-.01){this.orientation.x=0;}else{this.orientation.x=vel.x/Math.abs(vel.x);this.direction.x=Math.cos(rot);}
if(vel.y<.01&&vel.y>-.01){this.orientation.y=0;}else{this.orientation.y=vel.y/Math.abs(vel.y);this.direction.y=Math.sin(rot);}
vel.x=this.velocity.x.clamp(this.minVelocity.x,this.maxVelocity.x);vel.y=this.velocity.y.clamp(this.minVelocity.y,this.maxVelocity.y);if(vel.x!==this.velocity.x||vel.y!==this.velocity.y){this.velocity=vel;}
...
this._super();},update:function(){if((this._calcGoal&&!GOAL_REFRESH_FPS)||(this._calcGoal&&this.goals.length==0))this.goalFind();this._super();},goalFind:function(){if(!this._calcGoal)return
var vx=this.velocity.x;var vy=this.velocity.y;var nextPos=new Box2D.Common.Math.b2Vec2(this.position.x+vx+(this.PLAYFIELD_SCALE.x>>1),this.position.y+vy+(this.PLAYFIELD_SCALE.y>>1));var nextGridPos=new Box2D.Common.Math.b2Vec2(0,0);nextGridPos.x=isNaN(nextPos.x)?0:parseInt(nextPos.x/this.GOAL_RESOLUTION.x);nextGridPos.y=isNaN(nextPos.y)?0:parseInt(nextPos.y/this.GOAL_RESOLUTION.y);nextGridPos.x=Math.round(nextGridPos.x<this.scaleWidth?this.scaleWidth:nextGridPos.x>=this.grid.width-this.scaleWidth?this.grid.width-this.scaleWidth:nextGridPos.x);nextGridPos.y=Math.round(nextGridPos.y<this.scaleHeight?this.scaleHeight:nextGridPos.y>=this.grid.height-this.scaleHeight?this.grid.height-this.scaleHeight:nextGridPos.y);if(this.goals.length>0){var goalX=this.goals[0].gridPos.x;var goalY=this.goals[0].gridPos.y;this.path=this.goalfinder.findPath(nextGridPos.x,nextGridPos.y,goalX,goalY,this.grid.clone())
...
if(pathLen>1&&inRadius){var xSign=(distX!=0?distX/Math.abs(distX):(Math.abs(vx)!=0?vx/Math.abs(vx):1));var ySign=(distY!=0?distY/Math.abs(distY):(Math.abs(vy)!=0?vy/Math.abs(vy):1));vx=Math.abs(vx)*((dist==0?1:Math.abs(distX)/dist)*xSign);vy=Math.abs(vy)*((dist==0?1:Math.abs(distY)/dist)*ySign);vx=(vx+(goalstrength?goalstrength*xSign:0))*this.goalAvoidSign;vy=(vy+(goalstrength?goalstrength*ySign:0))*this.goalAvoidSign;var vel=new Box2D.Common.Math.b2Vec2(vx,vy);if(vel.x!==this.velocity.x||vel.y!==this.velocity.y){this.velocity=vel;}}}
...
(function()
{JSO.Client.Model.Player=JSO.Client.Model.Actor.extend({constructor:JSO.Client.Model.Player,construct:function(id,typename,metatype){this._super(id,typename||'player');},update:function(){var rotation=this.rotation;this.orientation=new Box2D.Common.Math.b2Vec2(Math.cos(rotation*(Math.PI/180)),Math.sin(rotation*(Math.PI/180)));var rotation=Math.floor(Math.abs(rotation)/360)>0?rotation+((rotation>0?-1:1)*(Math.floor(Math.abs(rotation)/360)*360)):rotation;rotation+=this.angularImpulse;this.rotation=(rotation<0?rotation+360:(rotation>360?rotation-360:rotation));this.angularImpulse=this.angularImpulse-((this.angularImpulse<0?-1:1)*this.angularDamping/GAME_REFRESH_FPS);this.velocityNormal=this.velocity.Copy();this.velocityNormal.Normalize();this._super();}});})();﻿
(function()
...
this._super(model);},update:function(model){if(model==undefined||this.parent==undefined)return;if(this.parent.el==undefined||!this.parent.visible)return;this.parent.el.attrs.crop.x=model.animstep*model.scale.x;this.parent.el.attrs.crop.y=model.currentrow*model.scale.y;if(isNumeric(model.rotationmod)){this.parent.el.setRotationDeg(model.rotationmod);}
this._super(model);}});})();﻿
(function()
