...
(function()
{
	JSO.Client.Model.Actor = JSO.Client.Model.Object.extend({
		...
		update: function(){
			...
			vel.x = vel.x.clamp(-this.maxVelocity.x, -this.minVelocity.x);
			vel.y =  vel.y.clamp(-this.minVelocity.y, this.maxVelocity.y);
			vel.x =  vel.x.clamp(-this.maxVelocity.x, this.minVelocity.x);
			vel.y =  vel.y.clamp(-this.maxVelocity.y, this.minVelocity.y);
			this.velocity = vel;
			
			if(this.acceleration) vel = vel.Add(this.acceleration);
			*/
		...
		goalFind: function(){
			...
			if(this.goals.length > 0){
				...
				if(pathLen > 1 && inRadius){
					...
					this.velocity = new Box2D.Common.Math.b2Vec2(vx,vy);
				}
			}
			delete nextPos;
